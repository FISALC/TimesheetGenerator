...
namespace TimesheetGenerator
{
    public class TimesheetService
    {
        ...
        // Employee Name constant is used for logic but user requested blank in top info, keeping consistent with Program.cs logic
        ...
        public string GenerateTimesheet(int year, int month, string approverName, string approverRole)
        {
        ...
        private void BuildLayout(IXLWorksheet ws, int year, int month)
        {
            ...
            try 
            {
                ...
                var resourceName = "TimesheetGenerator.logo.png";
            ...
            {
                ws.Cell("A1").Value = "Error loading logo";
            }
        ...
        private void FillTimesheetData(IXLWorksheet ws, int year, int month, string approverName, string approverRole)
        {
            ...
            currentRow += 8; // Offset for next block (Reverted to original as classification is removed from first block)
        ...
        private void CreateSignatureBlock(IXLWorksheet ws, int startRow, string title, string name, string role, string label1, string label2, string label3, DateTime date, bool includeClassification)
        {
            ...
            if (includeClassification)
            {
                ...
                // No merge needed as it's just text
            }
        }
    }
}
